...
在控制台发送消息的时候，可以添加很多参数，而消息的持久化是要配置一个`properties`，把它改成2：
...
- 非持久化在存数据的时候会出现阻塞情况，数据太多了mq要进行存入磁盘（pageout），在pageout的过程中mq是完全不处理消息的，性能下降

- 但是数据持久化速度也会慢一点，因为还要往磁盘写就会慢
- 所以我们引入LazyQueue
...
而在3.12版本之后，LazyQueue已经成为所有队列的默认格式。因此官方推荐升级MQ为3.12版本或者所有队列都设置为LazyQueue模式。【耶我就是诶(
•̀ ω •́ )y】
...
一般reject方式用的较少，除非是**消息格式有问题**，那就是开发问题了。因此大多数情况下我们需要将消息处理的代码通过`try catch`
...
spring:
  rabbitmq:
    listener:
      simple:
        acknowledge-mode: none # 不做消费者确认处理，投出去了就立刻删除
```
...
当然，上述极端情况发生的概率还是非常低的，不过不怕一万就怕万一。为了应对上述情况Spring又提供了*
...
因此，我们必须想办法保证消息处理的幂等性。这里给出两种方案：**【怎么这么像锁的】**
...
**注意：**
RabbitMQ的消息过期是基于追溯方式来实现的，也就是说**当一个消息的TTL到期以后不一定会被移除或投递到死信交换机，而是在消息恰好处于队首时才会被处理。
**
