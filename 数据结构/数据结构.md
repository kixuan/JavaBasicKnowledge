...
- 树是一种非线性数据结构，由节点（nodes）组成。
- 包括一个称为根（root）的节点以及零个或多个子树，这些子树也是树，并且它们的根节点被连接到根节点。
...
void updateNode(TreeNode* root, int oldValue, int newValue) {
    TreeNode* nodeToUpdate = search(root, oldValue);
...
- 在数组中，位置 `i` 的节点的左子节点是 `2*i + 1`，右子节点是 `2*i + 2`，父节点是 `(i-1)/2`。
...
- 从一组节点（通常是字符频率）开始，每次选择两个最小的节点合并，直到只剩下一个节点（树的根节点）。
- 构造过程中创建的二叉树即为哈夫曼树。
...
void generateHuffmanCodes(struct HuffmanNode* root, string str, 
...
void BFS(const Graph& g, int start) {
    ...
    while (!queue.empty()) {
        ...
        AdjListNode* node = g.array[s].head;
        while (node != nullptr) {
            if (!visited[node->dest]) {
